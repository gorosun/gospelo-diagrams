...
import { enrichDiagram, generateMeta, renderStandalone, renderSvg, renderPreviewHtml, createBuilder } from '../src/index';
...
function showHelp(): void {
  console.log(`
gospelo-diagrams CLI

Usage:
  gospelo-diagrams <command> <input.json> [output] [options]
...
Examples:
  ...
  gospelo-diagrams --output html --diagram system.json
  gospelo-diagrams --output svg --diagram system.json
  gospelo-diagrams --open --diagram system.json
  gospelo-diagrams --insert-above api_gateway --node '{"id":"waf","icon":"aws:waf","label":"WAF"}' --diagram system.json
  gospelo-diagrams --insert-below lambda --node '{"id":"db","icon":"aws:dynamodb","label":"DynamoDB"}' --diagram system.json
  gospelo-diagrams --update-node lambda --node '{"label":"Updated Lambda"}' --diagram system.json
  gospelo-diagrams --remove-node old_node --diagram system.json
  gospelo-diagrams --align-top api --nodes 'lambda,dynamodb,s3' --diagram system.json
  gospelo-diagrams --align-left api --nodes 'waf,cloudfront' --diagram system.json

  # Icon catalog
  gospelo-diagrams --icon-catalog              # List all catalog paths
  gospelo-diagrams --icon-catalog aws          # List AWS catalog path
  gospelo-diagrams --icon-catalog --open       # Open all catalogs in browser
  gospelo-diagrams --icon-search lambda        # Search for 'lambda' across all providers
  gospelo-diagrams --icon-search "s3 storage"  # AND search with multiple terms
...
if (options.iconCatalog) {
  ...
  if (options.iconCatalogProvider) {
    ...
    console.log(`[AI] To search icons: gospelo-diagrams --icon-search "<keyword>"`);
  } else {
    // List all catalogs
  ...
  for (const [provider, categories] of Object.entries(iconCatalog)) {
    ...
    for (const icons of Object.values(categories)) {
      for (const icon of icons) {
        ...
        const searchText = `${iconId} ${icon.displayName} ${icon.category}`.toLowerCase();
    ...
    if (matches.length > 0) {
      const version = iconSourceVersions[provider];
      const commitShort = version?.commitId ? version.commitId.slice(0, 7) : 'N/A';
...
switch (command) {
  case 'enrich': {
    if (positionalArgs.length < 1) {
      ...
      console.error('Usage: gospelo-diagrams enrich <input.json> [output.json]');
      process.exit(1);
    }
  ...
  case 'meta': {
    if (positionalArgs.length < 1) {
      console.error('Error: Input file required');
      console.error('Usage: gospelo-diagrams meta <input.json>');
      process.exit(1);
    }
  ...
  case 'render': {
    if (positionalArgs.length < 1) {
      console.error('Error: Input file required');
      console.error('Usage: gospelo-diagrams render <input.json> [output.html]');
      process.exit(1);
    }
  ...
  case 'svg': {
    if (positionalArgs.length < 1) {
      console.error('Error: Input file required');
      console.error('Usage: gospelo-diagrams svg <input.json> [output.svg]');
      process.exit(1);
    }
  ...
  case 'preview': {
    if (positionalArgs.length < 1) {
      console.error('Error: Input file required');
      console.error('Usage: gospelo-diagrams preview <input.json> [output.html]');
      process.exit(1);
    }
  ...
  case 'eval': {
    if (positionalArgs.length < 2) {
      ...
      console.error('Usage: gospelo-diagrams eval <input.json> \'<expression>\' [output.json]');
  ...
  case 'edit': {
    if (positionalArgs.length < 2) {
      ...
      console.error('Usage: gospelo-diagrams edit <input.json> <patch.json> [output.json]');
      process.exit(1);
    }
  ...
  case 'add-node': {
    if (positionalArgs.length < 2) {
      ...
      console.error('Usage: gospelo-diagrams add-node <input.json> <node.json|JSON> [output.json]');
      process.exit(1);
    }
  ...
  case 'remove-node': {
    if (positionalArgs.length < 2) {
      ...
      console.error('Usage: gospelo-diagrams remove-node <input.json> <node-id> [output.json]');
      process.exit(1);
    }
  ...
  case 'move-node': {
    if (positionalArgs.length < 4) {
      ...
      console.error('Usage: gospelo-diagrams move-node <input.json> <node-id> <x> <y> [output.json]');
      process.exit(1);
    }
  ...
  case 'add-connection': {
    if (positionalArgs.length < 3) {
      ...
      console.error('Usage: gospelo-diagrams add-connection <input.json> <from> <to> [output.json]');
      process.exit(1);
    }
  ...
  case 'remove-connection': {
    if (positionalArgs.length < 3) {
      console.error('Error: Input file, from, and to required');
      console.error('Usage: gospelo-diagrams remove-connection <input.json> <from> <to> [output.json]');
      process.exit(1);
    }
  ...
  case 'embed': {
    if (positionalArgs.length < 1) {
      console.error('Error: Input file required');
      console.error('Usage: gospelo-diagrams embed <input.json|input.html|input.svg> [--obfuscate]');
