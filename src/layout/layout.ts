...
import { Node, DiagramDefinition, ComputedNode, LayoutDirection } from '../core/types';
...
export function computeLayout(diagram: DiagramDefinition): ComputedNode[] {
  return diagram.nodes.map((node, index) => computeNodeLayout(node, index, null));
}
...
function computeNodeLayout(
  ...
  if (node.children && node.children.length > 0) {
    computed.children = node.children.map((child, i) =>
      computeNodeLayout(child, i, computed)
    );
  }
