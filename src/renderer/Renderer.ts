...
${gradients.join('\n')}
  ...
  private renderIconNode(node: ComputedNode): string {
    ...
    const onerrorHandler = iconUrl
      ? `onerror="console.error('[gospelo-diagrams] Icon load failed:', {id:'${node.id}',icon:'${node.icon}',x:${x},y:${y},url:'${iconUrl}'}); this.style.display='none'"`
      : '';
  ...
  private renderCompositeNode(node: ComputedNode): string {
    ...
    const icons = (node.icons || []).map((iconRef, i) => {
      ...
      if (iconUrl) {
        const onerrorHandler = `onerror="console.error('[gospelo-diagrams] Icon load failed:', {id:'${node.id}',icon:'${iconRef.icon}',x:${x + iconX},y:${y + iconY},url:'${iconUrl}'}); this.style.display='none'"`;
    ...
    await Promise.all(
      Array.from(urls).map(async (url) => {
        try {
          ...
          if (!response.ok) {
            console.error(`[gospelo-diagrams] Failed to fetch icon: ${url} (${response.status})`);
            return;
          }
          ...
          console.error(`[gospelo-diagrams] Error fetching icon: ${url}`, error);
        }
      })
    );
...
Generated with gospelo-diagrams
